...
export class WinnerCar {
    private getCarIcon(color: string): string {
        return `<svg fill="${color}" height="85px" viewBox="0 -43.92 122.88 122.88" style="position: absolute" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" transform="matrix(-1, 0, 0, 1, 0, 0)">
    ...
    public render(car: CarWinner, number: number): Element {
        ...
        div.innerHTML = `<div class="winners__cars">
            <div>${number}</div>
            <div class="winners__svg">${this.getCarIcon(car.color)}</div>
            <div>${car.name}</div>
            <div>${car.wins}</div>
            <div>${car.time}</div>
        </div>`;
